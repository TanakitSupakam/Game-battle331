...
function veigAttack(){
	...
	if (attackChoice == 1){
		...
		if (hitChance <=7){
			...
			bottomRow.innerHTML += "<br>Reiner missed!";
		}
		...
		if (hitChance <=5){
			...
			bottomRow.innerHTML += "<br>Reiner hit with you a basic Blast, doing " + dmg + " damage. You now have " + megaHP + "hp.";
			var megaHPBarWidth = (megaHP/100)*300;
			megaManHP.style.width = megaHPBarWidth + "px";
		} else {
			bottomRow.innerHTML += "<br>Eren missed!";
		}
	} else {
		var hitChance = Math.round(Math.random()*10);
		if (hitChance <=3){
...
function blast(){
	...
	if (hitChance <=7){
		...
		bottomRow.innerHTML = "You hit Reiner with your blast, doing " + dmg + " damage. Reiner now has " + veigarHP + "hp.";
		var veigHPBarWidth = (veigarHP/100)*300;
		veigHP.style.width = veigHPBarWidth + "px";
	} else {
		bottomRow.innerHTML = "Reiner missed!";
	}
	if (veigarHP == 0){
